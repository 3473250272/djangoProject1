...
def my_order(request):
    # 获取我的订单，当前登录的客户id
    queryset = models.Order.objects.filter(customer_id=request.user_data.user_id, active=1).order_by('-id')

    pager = Pagination(request, queryset)
    return render(request, 'web/my_order.html', {"pager": pager})
...
class MyOrderModelForm(BootStrapForm, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for item in queryset:
            ...
            price_count_list.append([item.count, "满{}，{}元/条".format(item.count, unit_price), unit_price])
            text_count_list.append("满{}，{}元/条".format(item.count, unit_price))
...
def my_order_add(request):
    ...
    try:
        with transaction.atomic():
            cus_object = models.Customer.objects.filter(id=request.user_data.user_id).select_for_update().first()
            ...
            form.instance.customer_id = request.user_data.user_id
            form.save()

            # 5.客户账户扣款
            models.Customer.objects.filter(id=request.user_data.user_id).update(balance=F("balance") - real_price)
...
def my_order_cancel(request, pk):
    """ 撤单"""
    order_object = models.Order.objects.filter(id=pk, active=1, status=1, customer=request.user_data.user_id).first()
    if not order_object:
    ...
    try:
        with transaction.atomic():
            models.Customer.objects.filter(id=request.user_data.user_id).select_for_update().first()

            # 1.订单状态变化为 (5, "已撤单"),
            models.Order.objects.filter(id=pk, active=1, status=1, customer=request.user_data.user_id).update(status=5)

            # 2.归还余额
            models.Customer.objects.filter(id=request.user_data.user_id).update(
