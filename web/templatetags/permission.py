...
def edit_permission(request, name, *args, **kwargs):
    ...
    if param:
        ...
        print(param)
        tpl = """<a href="{}?{}" class="btn btn-primary btn-sm">编辑</a>""".format(url, filter_string)
        return mark_safe(tpl)

    tpl = """<a href="{}" class="btn btn-primary btn-sm">编辑</a>""".format(url)
    return mark_safe(tpl)
...
def delete_permission(request, name, *args, **kwargs):
    ...
    pk = kwargs.get('pk')
    tpl = """
    <a data-cid="{}" class="btn btn-danger btn-sm delete-btn">删除</a>
    """.format(pk)
    return mark_safe(tpl)
...
def delete_url_permission(request, name, *args, **kwargs):
    ...
    url = reverse(name, args=args, kwargs=kwargs)
    tpl = """
    <a href="{}" class="btn btn-danger btn-sm delete-btn">删除</a>
    """.format(url)
    return mark_safe(tpl)
